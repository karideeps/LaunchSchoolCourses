WINNING_TOTAL = 5
FIRST_PLAYER = 'player' # valid enteries are 'choose', 'player', or 'computer'
...
def acquire_ready_for_round(round_number)
  loop do
    prompt "Are you ready for round #{round_number}?" \
           " Enter 'y' or 'yes' when ready!"
    answer = gets.chomp.downcase
    break if valid_ready_for_round_answer?(answer)
    prompt "Please enter 'y' or 'yes' when you are ready!"
  end
end

def valid_ready_for_round_answer?(answer)
...
def computer_moves_piece!(board)
  computer_choice = find_best_square(board, COMPUTER_MARKER)
  computer_choice = find_best_square(board, PLAYER_MARKER) if !computer_choice
  computer_choice = 5 if !computer_choice && board[5] == INITIAL_MARKER
  computer_choice = empty_squares(board).sample if !computer_choice

  board[computer_choice] = COMPUTER_MARKER
end

def find_best_square(board, marker)
  choice = nil
...
loop do # main game loop
  ...
  loop do # round loop
    acquire_ready_for_round(round_number)
